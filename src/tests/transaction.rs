...
mod transaction_tests {
    ...
    fn test_invalid_transaction() {
        ...
        let invalid_pubk = Transaction {
            sender_pubk: encode_pubk_to_hex(identity::Keypair::generate_ed25519().public()),
            ..valid_txn.clone()
        };
        assert!(matches!(
        ...
        let invalid_siglen = Transaction {
            sig: encode_bytes_to_hex(ZERO_U32),
            ..valid_txn.clone()
        };
        assert!(matches!(
        ...
        let invalid_sig = Transaction {
            sig: encode_bytes_to_hex(ZERO_U64),
            ..valid_txn.clone()
        };
        assert!(matches!(
