...
pub fn decode_pubk(pubk_hex: &String, n_bytes : usize) -> Result<PublicKey, String>{
    ...
    if pubk_u8s.len() != n_bytes {
        return Err(format!("Unexpected number of bytes in public key. Expected: {} , Actual: {}", n_bytes, pubk_u8s.len()))
    };
...
pub fn decode_hex(hash_hex: &String, n_bytes : usize) -> Result<Vec<u8>, String>{
    ...
    if hash_u8s.len() != n_bytes {
        return Err(format!("Unexpected number of bytes in hex-string. Expected: {}, Got: {}", n_bytes, hash_u8s.len()))
    }
    Ok(hash_u8s)
}