...
pub const ZERO_U8_32 : [u8; 32] = [0; 32];
...
pub struct Block {
    ...
    // 256-bit hash of the above fields, uniquely identifying this block
    pub hash: [u8; 32],
...
impl Block {
  ...
  pub fn mine_block(idx: u64, data: &str, prev_hash: [u8; 32]) -> Block {
        ...
        info!("mining block for:\n
                ...
                , idx, data, now, _32bytes_to_hexstr(prev_hash));
  ...
  fn compute_hash (idx: u64, data: &str, timestamp: i64, prev_hash: [u8; 32], nonce: u64)  -> [u8; 32] {
        ...
        hasher.finalize() // Sha256 -> GenericArray<u8, U32>
  ...
  pub fn valid_block(last_block: &Block, block: &Block) -> bool {
    ...
    if block.hash != Block::hash_block(&block) {
        eprintln!("Invalid block! block with idx {} stores a hash {} different from its real hash {}"
            , last_block.idx, _32bytes_to_hexstr(block.hash), _32bytes_to_hexstr(Block::hash_block(&block))) ;
        return false
    }
    // * proof-of-work check:
...
impl std::fmt::Display for Block {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        write!(f, "Block {{\n\t idx: {}, \n\t data: {}, \n\t hash: {}, \n\t date-time: {}, \n\t nonce: {} \t}}"
                , self.idx, self.data, _32bytes_to_hexstr(self.hash), date_time, self.nonce)
    }
}
